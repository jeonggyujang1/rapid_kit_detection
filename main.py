...
#crope_image_from_yolo_result(results[i])
...
def c_value_line_from_img(img):
    #rgb_sum = rotated_img.sum(axis=2)
    if img.shape[0]>img.shape[1]:
        value_line = img.mean(axis=1)         
    else :
        value_line = img.mean(axis=0)
...
def result_image_gen_v3(image_rgb, cropped_imgs, image_path, figsize=10):
    ...
    for i in range(len(cropped_imgs)):
        value_line, colors = c_value_line_from_img(cropped_imgs[i]['img'])
        value_lines.append(value_line)
        colors_list.append(colors)
    ...
    plt.savefig(f'./analysis_results/{file_name}_1.{ext}')
    print(f'./analysis_results/{file_name}_original.{ext} (saved)')


    suplot_x_kit = round(len(cropped_imgs)//3+0.51)
    subplot_x = suplot_x_kit + round(len(cropped_imgs)//3+0.51)
    subplot_y = 6
    ...
    for img_ind in range(len(cropped_imgs)):
        ...
        plt.subplot(subplot_x, subplot_y, suplot_x_kit*subplot_y + 2*img_ind+1)
        ...
        plt.subplot(subplot_x, subplot_y, suplot_x_kit*subplot_y + 2*img_ind+2)
        ...
        for peak_ind, peak in enumerate(peaks):
            if rotation==0:
                ...
                plt.subplot(subplot_x, subplot_y, suplot_x_kit*subplot_y + 2*img_ind+1)
                rect = patches.Arrow(x_peak-strip_width//2,y_peak,strip_width,0 ,alpha = 0.3, linewidth=0.5, edgecolor='magenta', facecolor='None')
                plt.gca().add_patch(rect)
                ...
                print(rotation, x_peak,y_peak)
                plt.subplot(subplot_x, subplot_y, suplot_x_kit*subplot_y + 2*img_ind+1)
                rect = patches.Arrow(x_peak,y_peak-strip_width//2,0,strip_width ,alpha = 0.3, linewidth=0.5, edgecolor='magenta', facecolor='None')
                plt.gca().add_patch(rect)
                plt.annotate(f"{smoothed[peak]:.0f}", (x_peak, y_peak), c='magenta', fontsize=fontsize)
                #background_x.append(strip_center[0] - strip_heigth//2 + peak)
                #background_y.append(strip_center[1])
        ...
        if rotation==0:
            ...
            rect = patches.Arrow(x_peak_-strip_width//2,y_peak_,strip_width,0 ,alpha = 0.3, linewidth=0.5, edgecolor='green', facecolor='None')
            plt.gca().add_patch(rect)
        else:
            plt.subplot(subplot_x, subplot_y, suplot_x_kit*subplot_y + 2*img_ind+1)
            plt.annotate(f"{smoothed[x_peak_]:.0f}", (x_peak_, y_peak_-strip_width//3), c='green', fontsize=fontsize)
            rect = patches.Arrow(x_peak_,y_peak_-strip_width//2,0,strip_width ,alpha = 0.3, linewidth=0.5, edgecolor='green', facecolor='None')
            plt.gca().add_patch(rect)
    #return y_
    plt.show()
    file_name = image_path.split('/')[-1]
    ext = file_name.split('.')[-1]
    file_name = "".join(file_name.split('.')[:-1])
    plt.savefig(f'./analysis_results/{file_name}_cropped.{ext}')
    print(f'./analysis_results/{file_name}_cropped.{ext} (saved)')
...
def find_xy_in_roi(roi_yx1,roi_yx2,yxs, base = 'center'):
    print(f'roi_yx1 = {roi_yx1}')
    print(f'roi_yx2 = {roi_yx2}')
    print(f'yxs = {yxs}')
    if base=='center':
        origin = [int(sum(x)/2) for x in zip(roi_yx1,roi_yx2)]
    else :
        orgin = [0,0]
    yx_list = []
    for yx in yxs:
        x = yx[1]
        y = yx[0]
        if roi_yx1[0] < y and roi_yx2[0] > y:
            if roi_yx1[1] < x and roi_yx2[1] > x:
                yx_list.append(yx)
    if len(yx_list)!=0: 
        result, result_ind = find_closest_coordinate(yx_list,origin)
...
results = model.predict(source=img_files, save=False, save_txt=False)
...
#for result in results[5:7]: #yolo 결과들 중 한 이미지에 대한 결과
    ...
    if result.boxes is not None:
        ...
        for i in range(len(cropped_kits)):
            ...
            kit_yolo_result = model.predict(source=transformed_kit, save=False, save_txt=False)
            ...
            if len(t_cropped_strips) != 0:
                ...
                if len(t_cropped_strips)>1:
                    ...
                    t_center,t_center_ind = find_xy_in_roi(kit_origin_t,[kit_center_t[0]+kit_heigth_t//2,kit_center_t[1]+kit_width_t//2],xys)
                else :
